...
fun hmsLogin(activity: Activity, result: MethodChannel.Result) {
    val read = intArrayOf(
...
fun getSteps(call: MethodCall, activity: Activity, result: MethodChannel.Result) {
    ...
    try {
        runBlocking {
            val data = queryHiHealth(activity, hiHealthDataQuery)
...
fun getDistance(call: MethodCall, activity: Activity, result: MethodChannel.Result) {
    val (startTime, endTime) = getTime(call)
...
suspend fun queryHiHealth(activity: Activity, query: HiHealthDataQuery) = suspendCoroutine<Map<String, Int>> { cont ->
...
fun getTime(call: MethodCall): Pair<Long, Long> {
